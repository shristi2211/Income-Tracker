...
export default function ConsentHistoryPanel({ onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center sm:justify-start p-0 sm:p-6">
      ...
      {/* Panel — ARCompli style */}
      <div className="relative z-10 bg-white rounded-t-2xl sm:rounded-2xl w-full sm:w-[360px] shadow-2xl shadow-black/30 flex flex-col max-h-[90vh] overflow-hidden text-gray-800">
        ...
        <div className="overflow-y-auto flex-1 px-5 pt-5 pb-3">
            ...
            {COOKIE_TYPES.map(({ key, label, desc, required }) => (
              <div key={key} className="flex items-start gap-3 cursor-pointer" onClick={() => toggleCookie(key)}>
                <div className="mt-0.5 shrink-0">
                  {required ? (
                    /* Necessary — always on, teal circle check */
                    ...
                    <div className="w-5 h-5 rounded border-2 border-teal-500 bg-white flex items-center justify-center">
                      <svg width="11" height="8" viewBox="0 0 11 8" fill="none">
                        <path d="M1.5 4L4 6.5L9.5 1" stroke="#14b8a6" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                    </div>
                  ) : (
                ...
                <div>
                  <p className={`text-[13px] font-semibold leading-tight ${required ? 'text-gray-800' : 'text-gray-700'}`}>
                    {label}
                ...
                return (
                  <div key={email} className="mb-4">
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-[13px] font-semibold text-gray-800 break-all">{email}</span>
                      <button
                        onClick={() => handleRevoke(email)}
                        className="ml-2 shrink-0 text-[11px] font-semibold text-red-500 border border-red-400 rounded px-2 py-0.5 hover:bg-red-50 transition"
                      >
                        Revoke
                      </button>
                    </div>
                    ...
                    {/* Status */}
                    <p className="text-[12px] text-teal-600 font-medium flex items-center gap-1 mb-0.5">
                      <svg width="12" height="9" viewBox="0 0 12 9" fill="none">
                        <path d="M1 4.5L4 7.5L11 1" stroke="#0d9488" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                      Active consent since {grantedRec ? new Date(grantedRec.timestamp).toLocaleDateString('en-IN') : '—'}
                    </p>
                    <p className="text-[11px] text-gray-400 mb-1">Purpose: Marketing and product updates</p>

                    {/* Recent Activity */}
                    <p className="text-[11px] text-gray-500 font-medium mb-1">Recent Activity:</p>
                    {recentActivity && (
          ...
          <div className="mb-3">
            ...
            {records.length === 0 ? (
              <p className="text-[12px] text-gray-400 italic">No history yet. History is recorded when you log in.</p>
            ) : (
                  ...
                  .map(rec => (
                    <div key={rec.id} className="text-[12px]">
                      <div className="flex justify-between">
                        <span className={`font-semibold ${rec.action === 'granted' ? 'text-teal-600' : 'text-red-500'}`}>
                          {rec.action === 'granted' ? 'granted' : 'revoked'} (email)
                        </span>
                        <span className="text-gray-400">{formatDate(rec.timestamp)}</span>
                      </div>
                      <p className="text-teal-600">Email: {rec.email}</p>
                    </div>
                  ))}
              </div>
            )}
            ...
            <button
              ...
              className="mt-3 w-full border border-gray-300 rounded-lg py-2 text-[12px] font-medium text-gray-600 hover:bg-gray-50 hover:border-gray-400 transition flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
            >
              <Download size={13} />
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-t border-gray-100">
          <button
            ...
            className="flex-1 border border-gray-300 rounded-lg py-2 text-[13px] font-semibold text-gray-600 hover:bg-gray-50 transition"
          >
            Reset All
          </button>
          <button
            onClick={handleSave}
            className="flex-1 bg-teal-500 hover:bg-teal-600 text-white rounded-lg py-2 text-[13px] font-semibold transition shadow-sm shadow-teal-200"
          >
            Save &amp; Close
          </button>
        </div>
